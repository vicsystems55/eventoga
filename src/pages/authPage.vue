<template>
  <div class="min-h-screen ambient-bg auth-wrapper">
    ...
    <div class="fixed inset-0 overflow-hidden pointer-events-none">
      ...
      <div class="absolute -bottom-40 -left-40 w-96 h-96 bg-purple-500/20 rounded-full blur-3xl animate-pulse-slower">
      </div>
      <div
        class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-orange-500/5 rounded-full blur-3xl">
      </div>
    </div>
    ...
    <div class="relative z-20 w-full px-4 py-6 md:px-8">
      <div class="mx-auto max-w-7xl">
        <router-link to="/"
          class="inline-flex items-center gap-2 text-white/70 hover:text-orange-500 transition-all duration-300 group">
    ...
    <div class="relative z-10 flex items-center justify-center px-4 py-8 md:py-12">
      <div class="w-full max-w-md">
        ...
        <div v-if="errorMessage"
          class="mx-6 mt-4 rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-300">
          {{ errorMessage }}
        </div>

        <!-- Auth Card -->
        <div
          class="relative rounded-3xl border border-white/10 bg-black/40 backdrop-blur-xl shadow-2xl overflow-hidden glow-mix">
          ...
          <div class="absolute inset-0 rounded-3xl pointer-events-none">
            <div
              class="absolute -inset-[1px] bg-gradient-to-r from-orange-500/20 via-purple-500/20 to-orange-500/20 rounded-3xl blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-500">
            </div>
          </div>
          ...
          <div class="relative px-6 pt-8">
            <div class="flex gap-2 p-1 rounded-xl bg-white/5 border border-white/10">
              <button @click="activeTab = 'login'"
                class="relative flex-1 py-3 px-4 rounded-lg font-bold transition-all duration-300" :class="activeTab === 'login'
                  ? 'text-white bg-gradient-to-r from-orange-500 to-orange-600 shadow-lg glow-orange'
                  : 'text-gray-400 hover:text-white hover:bg-white/5'">
                <span class="relative z-10">Login</span>
                <div v-if="activeTab === 'login'" class="absolute inset-0 rounded-lg bg-orange-500/20 blur-sm -z-0">
                </div>
              </button>
              <button @click="activeTab = 'register'"
                class="relative flex-1 py-3 px-4 rounded-lg font-bold transition-all duration-300" :class="activeTab === 'register'
                  ? 'text-white bg-gradient-to-r from-purple-500 to-purple-600 shadow-lg glow-purple'
                  : 'text-gray-400 hover:text-white hover:bg-white/5'">
                <span class="relative z-10">Register</span>
                <div v-if="activeTab === 'register'" class="absolute inset-0 rounded-lg bg-purple-500/20 blur-sm -z-0">
                </div>
              </button>
            </div>
          </div>
              ...
              <form @submit.prevent="handleLogin" class="space-y-5">
                ...
                <div class="space-y-2">
                  ...
                  <div class="relative">
                    ...
                    <button type="button" @click="showPassword = !showPassword"
                      ...
                      <svg v-if="!showPassword" class="h-5 w-5 text-gray-500 hover:text-gray-300" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      ...
                      <svg v-else class="h-5 w-5 text-gray-500 hover:text-gray-300" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="flex items-center justify-between">
                  <label class="flex items-center gap-2 text-sm text-gray-400 cursor-pointer">
                    <input type="checkbox"
                      class="w-4 h-4 rounded border-purple-500/30 bg-black/50 text-orange-500 focus:ring-orange-500 focus:ring-offset-0">
                    <span>Remember me</span>
                  </label>
                  <a href="#" class="text-sm text-orange-500 hover:text-orange-400 transition-colors">Forgot
                    Password?</a>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                  <button
                    class="flex items-center justify-center gap-2 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white hover:bg-white/10 transition-all duration-300">
              ...
              <form @submit.prevent="handleRegister" class="space-y-4">
                ...
                <div class="space-y-2">
                  ...
                  <div class="relative">
                    ...
                    <button type="button" @click="showRegisterPassword = !showRegisterPassword"
                      class="absolute inset-y-0 right-0 pr-3 flex items-center">
                      <svg v-if="!showRegisterPassword" class="h-5 w-5 text-gray-500 hover:text-gray-300" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                          d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="space-y-2">
                  ...
                  <div class="relative">
                    ...
                    <button type="button" @click="showConfirmPassword = !showConfirmPassword"
                      class="absolute inset-y-0 right-0 pr-3 flex items-center">
                      <svg v-if="!showConfirmPassword" class="h-5 w-5 text-gray-500 hover:text-gray-300" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                          d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <label class="flex items-start gap-3 text-sm text-gray-400 cursor-pointer">
                  <input type="checkbox" required
                    class="mt-1 w-4 h-4 rounded border-purple-500/30 bg-black/50 text-orange-500 focus:ring-orange-500 focus:ring-offset-0">
                  <span>I agree to the <a href="#" class="text-orange-500 hover:text-orange-400">Terms of Service</a>
                    and <a href="#" class="text-orange-500 hover:text-orange-400">Privacy Policy</a></span>
                </label>
...
const handleRegister = async () => {
  ...
  try {
    ...
    const nextStep = response.next_step || '/dashboard'
